...
import pynps.configuration as configuration
# import .configuration as configuration
import pynps.functions as func

# external imports
from uuid import uuid4 as id_gen

def _get_config_info(maindir):
    """ setting the default folder and file for configurations """
    runing_from = func.get_pyinstaller()
    if func.get_system() == 'Linux':
        base_folder = os.getenv('HOME')
        config_file_folder = f"{base_folder}/.config/pyNPS"
        config_file = f"{config_file_folder}/settings.ini"
    elif func.get_system() == 'Windows':
        if runing_from == 'pi-onefile':
            # one file will have all the related directories in the same folder as the .exe
            base_folder = maindir
            config_file_folder = f"{base_folder}/pynps_config/"
            config_file = f"{config_file_folder}/settings.ini"
            ...
            if runing_from == 'python':
                config_file_folder = f"{base_folder}/Documents/pyNPS"
            else:
                config_file_folder = f"{base_folder}/Documents/pyNPSbin"
            
            config_file = f"{config_file_folder}/settings.ini"
    
    return config_file_folder,config_file

def _check_pkg2zip_and_wget_existance(pkg2zip_path: str, wget_path: str, config_file_folder: str) -> tuple:
    pkg2zip = func.check_pkg2zip(pkg2zip_path, config_file_folder)
    wget = func.check_wget(wget_path, config_file_folder)
...
def cli_main(maindir=""):   
    config_file_folder, config_file = _get_config_info(maindir)
    
    # a path is given in case the user decides to use a configuration file in disk
    # if the user don't create a file in the default configuration path or provide a file path, all the necessary default data will be pulled from the Config object
    conf = configuration.Configurations(path=config_file)
    
    # TODO add a flag so people that want to generate a config file can do it
    # config file is made optional by only rading data when the conf file exists
    # if os.path.isfile(config_file_path):
    if False: #force the non config file mode
        try:
            conf.read_from_file()
        except FileNotFoundError:
            func.create_folder(config_file_folder)
            conf.commit_to_file()
            
    dl_path = conf.download_folder.unrap(maindir) #DLFOLDER
    db_path = conf.database_folder.unrap(maindir) #DBFOLDER
    pkg2_path = conf.pkg2zip_location.unrap(maindir)
    wget_path = conf.wget_location.unrap(maindir)
    
    # TODO change the variable names in all file according to PEP8
    PKG2ZIP, WGET = _check_pkg2zip_and_wget_existance(pkg2_path, wget_path, config_file_folder)
#     # making vars
...
#     PKG2ZIP = fix_folder_syntax(
#         config['BinaryLocations']['pkg2zip_location'], maindir)
#     WGET = fix_folder_syntax(
#         config['BinaryLocations']['wget_location'], maindir)
...
#                 rich.print(
#                     "Tags can only be alphanumeric without spaces", style='dark_orange')
#                 input_tag = None
...
#             except:
#                 rich.print(
#                     "There are no saved download sessions to resume", style='dark_orange')
#                 sys.exit(0)
...
#             if input_tag is not None:
#                 rich.print(
#                     "There's no such tag in the download sessions", style='dark_orange')
...
#                 rich.print(
#                     "Error: you can't search while updating the database", style='red on black')
#                 sys.exit(1)
...
#             if get_xmas():
#                 rich.print(
#                     f"Done! [red]Happy Holidays!!![/red] 🎄✨", style='green')
#             else:
...
#             rich.print(
#                 "Error: No search term provided, you need to search for something, use -h for help", style='red on black')
#             sys.exit(1)

#         # checking for database's existense:
#         if os.path.isfile(f"{DBFOLDER}/pynps.db") is False:
#             rich.print(
#                 "Error: theres no database in your system, please update your database and try again, use -h for help", style='red on black')
#             sys.exit(1)
...
#             system, what_to_dl, args.search, reg, args.sort, DBFOLDER)

#         # test if the result isn't empty
#         if len(maybe_download) == 0:
#             rich.print(
#                 "No results found, try searching for something else or updating your database", style='dark_orange')
#             sys.exit(0)
...
#                 rich.print(
#                     "\tSuppose you have 10 files to select from:", style='bright_black')
#                 rich.print("\tTo download file 2, you type: 2",
#                            style='bright_black')
#                 rich.print(
#                     "\tTo download files 1 to 9, the masochist method, you type: 1,2,3,4,5,6,7,8,9", style='bright_black')
#                 rich.print(
#                     "\tTo download files 1 to 9, the cool-kid method, you type: 1-9", style='bright_black')
#                 rich.print(
#                     "\tTo download files 1 to 5 and files 8 to 10: 1-5,8-10", style='bright_black')
#                 rich.print(
#                     "\tTo download files 1, 4 and files 6 to 10: 1,4,6-10:", style='bright_black')
#                 rich.print(
#                     "\tTo download files 1, 4 and files 6 to 10, the crazy way, as the software doesn't care about order or duplicates: 10-6,1,4,6", style='bright_black')
...
#                             rich.print(
#                                 "Error: invalid syntax, please only use non-zero positive integer numbers", style='red on black')
#                             sys.exit(1)
...
#                     else:
#                         rich.print(
#                             "Error: syntax, please only use non-zero positive integer numbers", style='red on black')
#                         sys.exit(1)
#                 elif i.isdigit() == True:
...
#             rich.print(
#                 'Unable to download .pkg file. Do you have a internet connection?', style='red on black')
...
#             if i["SHA256"] == "":
#                 rich.print(
#                     'No checksum provided by NPS, skipping check', style='dark_orange')
#             else:
...
#                     rich.print(
#                         'Checksum not matching, pkg file is probably corrupted, delete it at your download folder and redownload the pkg', style='red on black')
#                     rich.print(
#                         f'Corrupted file is located at [bright_black]{loc}[/bright_black]', style='red on black')
#                     break  # skip file
#                 else:
...
#                         extraction_folder = f"{DLFOLDER}/Extracted/pspemu/ISO/[italic]game_name[/italic] [{i['Title ID']}].iso"
#                     elif cso_factor in [str(x) for x in range(1, 10)]:
#                         extraction_folder = f"{DLFOLDER}/Extracted/pspemu/ISO/[italic]game_name[/italic] [{i['Title ID']}].cso"
#                     elif args.eboot == True:
#                         extraction_folder = f"{DLFOLDER}/Extracted/pspemu/PSP/GAME/{i['Title ID']}"
#                 if i["Type"] == "DLCS":
#                     extraction_folder = f"{DLFOLDER}/Extracted/pspemu/PSP/GAME/{i['Title ID']}"
#                 if i["Type"] == "THEMES":
#                     extraction_folder = f"{DLFOLDER}/Extracted/pspemu/PSP/THEME/[italic]theme_name[/italic].PTF"
#                 if i["Type"] == "UPDATES":
#                     extraction_folder = f"{DLFOLDER}/Extracted/pspemu/PSP/GAME/{i['Title ID']}"
...
#                 rich.print(
#                     f"cso is only supported for PSP games, since you're extracting a {i['System']} {i['Type'][:-1].lower()} the compression will be skipped", style='dark_orange')
...
#                 delete = run_pkg2zip(
#                     dl_dile_loc, dl_location, PKG2ZIP, pkg2zip_args, extraction_folder, i, zrif)
#             else:
#                 delete = run_pkg2zip(
#                     dl_dile_loc, dl_location, PKG2ZIP, pkg2zip_args, extraction_folder, i)
...
#                 except:
#                     rich.print(
#                         f"Unable to delete file, you may want to it manually at [bright_black]{dl_dile_loc}[/bright_black]", style='red on black')
...
#                     f"[green on black][PKG2ZIP][/green on black][green] Attempting extraction")
#                 rich.print(
#                     "Skipping extraction since there's no pkg2zip binary in your system", style='red on black')
#             else:
...
#                     rich.print(
#                         f"PS3 pkg file moved to [bright_black]{pkg_new_location}[/bright_black]", style='green')
#                     delete = True
#                 except:
...
#                         os.remove(dl_dile_loc)
#                         rich.print(
#                             f"The compressed .pkg was deleted", style='green')
#                     except:
#                         rich.print(
#                             f"Unable to delete file, you may want to it manually at [bright_black]{dl_dile_loc}[/bright_black]", style='red on black')
...
#                     rich.print(
#                         f"RAP files aren't required for this game :^)", style='green')
#                 elif i['RAP'] == "MISSING" or i['RAP'] == "":
#                     rich.print(
#                         f"Unfortunatelly there are no RAP files available for this game on Nopaystation", style='dark_orange')
...
#                     if rap_state:
#                         rich.print(
#                             f"PS3 RAP file downloaded to: [bright_black]{rap_folder}[/bright_black]", style='green')
#                     else:
#                         rich.print(
#                             'Unable to download .rap file. Do you have a internet connection?', style='red on black')
